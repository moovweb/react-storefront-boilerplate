...
const CartItem = ({ classes, product, cart }) => {
  const remove = () => cart.remove(product)
  ...
  return (
    <Paper className={classes.root}>
      ...
      <Track event="removedFromCart" product={product}>
        <IconButton className={classes.remove} onClick={remove}>
          <CloseIcon />
        </IconButton>
      </Track>
    </Paper>
...
export default withStyles(styles)(inject(({ app }) => ({ cart: app.cart }))(observer(CartItem)))
